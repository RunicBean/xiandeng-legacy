...
func NewLog(conf LoggerConfig, isProd bool) *Logger {
...
func (l *Logger) SetServerType(serverType string) {
	l.Logger = l.Logger.With(zap.String("server", serverType))
}
...
func (l *Logger) ErrorTraceback(msg string, err error, params ...zapcore.ObjectMarshaler) {
	if params != nil {
		l.Error(msg, zap.Error(err), zap.Objects("params", params))
	} else {
		l.Error(msg, zap.Error(err))
	}
}

type SimpleParam string

func (s SimpleParam) MarshalLogObject(enc zapcore.ObjectEncoder) error {
	enc.AddString("value", string(s))
	return nil
}

type SimpleMapParam map[string]string

func (s SimpleMapParam) MarshalLogObject(enc zapcore.ObjectEncoder) error {
	for k, v := range s {
		enc.AddString(k, v)
	}
	return nil
